...
func main() {
	input := input.GetInput(2019, 13)[0]
	...
	fmt.Println(s)
	//	fmt.Println("part 1:", s.blockTiles())
	fmt.Println(s.robot.score)
}

func (s *arcade) blockTiles() int {
	count := 0
	for _, v := range s.screen.Data {
		if v == "+" {
			count++
		}
	}
...
type arcade struct {
	...
	robot     *robot
	ballPos   internal.Point
	paddlePos internal.Point
	movement  int
}

func (s *arcade) inputs() []int {
	return []int{s.movement}
}

func (s *arcade) String() string {
	return s.screen.String()
}
func (s *arcade) updateBall(x, y int) {
	if s.ballPos.X == x && s.ballPos.Y == y {
		return
	}
	s.ballPos = internal.Point{x, y}
...
func (s *arcade) updatePaddle(x, y int) {
	if s.paddlePos.X == x && s.paddlePos.Y == y {
		return
	}
	s.paddlePos = internal.Point{x, y}
	if s.paddlePos.X < s.ballPos.X {
		s.movement = 1
		return
	}
...
func (s *arcade) print(x, y, tile int) {
	o := " "
	switch tile {
	case 1:
		o = "#"
	...
	s.screen.Set(internal.Point{x, y}, o)
}
